...
function App() {
  ...
  const handleSuggestionClick = async (suggestion) => {
    ...
    setSuggestions([]);
    setShowSuggestions(false);
    setMarkers([{ name: suggestion.display_name, coordinates: [parseFloat(suggestion.lon), parseFloat(suggestion.lat)] }]);
  ...
  const handleMarkerClick = async (marker) => {
    ...
    await fetchWeather(marker.coordinates[1], marker.coordinates[0], marker.name);
  };
  ...
  return (
    <div className="weather-app-container">
      ...
      />
      <WorldMap geoUrl={geoUrl} markers={markers} onMarkerClick={handleMarkerClick} />
